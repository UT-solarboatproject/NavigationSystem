...
I2C_DRIVER = "SBC_LINUX_SMBUS"
# other I2C options
I2C_DEFAULT_CLK_KHZ = 100
I2C_DEFAULT_BUS_NUMBER = 1


class ina226:
    def __init__(
        self,
        ina226_addr=INA226_ADDRESS,
        i2c_bus_number=I2C_DEFAULT_BUS_NUMBER,
    ...
    def calibrate(self, rShuntValue=0.1, iMaxExcepted=2):
        ...
        calibrationValue = int(
            ((0.00512) / (self.currentLSB * self.rShunt))
...
def demo():
    try:
        ...
        while True:
            print(
                "Current: "
                + str(round(iSensor.readShuntCurrent(), 3))
                + "A"
                + ", Voltage: "
                + str(round(iSensor.readBusVoltage(), 3))
                + "V"
                + ", Power:"
                + str(round(iSensor.readBusPower(), 3))
                + "W"
            )
